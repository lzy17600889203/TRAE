...
@Injectable()
export class HydraulicCalculatorService {
  private readonly GRAVITY = 9.81;
  private readonly WATER_DENSITY = 1000;
  private readonly MAX_ITERATIONS = 100;
  private readonly CONVERGENCE_TOLERANCE = 0.0001;
  private readonly HIGH_PRESSURE_THRESHOLD = 60;
  private readonly MAX_ITERATIONS_FOR_ANOMALY = 500;
  ...
  calculate(
    ...
    const pipeMap = new Map<string, PipeData>();
    const pumpMap = new Map<string, PumpData>();
    const valveMap = new Map<string, ValveData>();
    ...
    let pressureHistory: Map<string, number[]> = new Map();
    ...
    while (!converged && iteration < this.MAX_ITERATIONS_FOR_ANOMALY) {
      ...
      if (iteration > 30 && !pressureOscillationDetected) {
        const oscResult = this.detectPressureOscillationWithHistory(
          nodeMap,
          pressureHistory,
          previousPressures,
        );
  ...
  private initialGuess(
    nodeMap: Map<string, NodeData>,
    pipeMap: Map<string, PipeData>,
    pumpMap: Map<string, PumpData>,
    valveMap: Map<string, ValveData>,
  ) {
  ...
  private calculateFlows(
    ...
    pipeMap.forEach((pipe) => {
      ...
      if (pipe.isBroken) {
        const effectiveFlow = this.calculateBrokenPipeFlow(
          pipe,
          startNode,
          endNode,
        );
      ...
      const pumpsOnPipe = Array.from(pumpMap.values()).filter(
        (p) => p.pipeId === pipe.id && p.isActive,
      );
      ...
      const valvesOnPipe = Array.from(valveMap.values()).filter(
        (v) => v.pipeId === pipe.id,
      );
  ...
  private calculatePipeResistance(pipe: PipeData): number {
    if (pipe.diameter <= 0) return Infinity;
  ...
  private calculateVelocities(pipeMap: Map<string, PipeData>) {
    pipeMap.forEach((pipe) => {
      if (pipe.diameter > 0) {
  ...
  private detectDeadEnds(
    ...
    const deadEndPipes: string[] = [];

    nodeMap.forEach((node, nodeId) => {
      if (node.isSource || node.isSink) return;

      const connectedPipes = Array.from(pipeMap.values()).filter(
  ...
  private detectPressureOscillationWithHistory(
    nodeMap: Map<string, NodeData>,
    pressureHistory: Map<string, number[]>,
    previousPressures: Map<string, number>,
  ): { detected: boolean; magnitude: number } {
  ...
  private detectPressureOscillation(
    ...
    if (previousPressures.size === 0) return false;
  ...
  private updateFlowDirection(pipeMap: Map<string, PipeData>) {
    pipeMap.forEach((pipe) => {
