export function computeDistanceMatrix(speciesList, characteristics, hasMissingData = false) {
    const featureNames = [...new Set(characteristics.map((c) => c.feature_name))];
    const n = speciesList.length;
    const speciesFeatures = new Map();
...
export function detectLongBranches(tree, threshold = 3) {
    const longNodes = [];
...
export function detectPolyphyletic(tree, speciesGroups) {
...
export function upgma(distanceMatrix) {
    const { labels, matrix, speciesIds } = distanceMatrix;
    const n = labels.length;
    const clusters = labels.map((name, i) => ({
        id: `leaf-${i}`,
        name,
    ...
    while (activeClusters.filter(Boolean).length > 1) {
        ...
        for (let k = 0; k < newSize - 1; k++) {
            if (activeClusters[k]) {
                const weightedDist = (currentMatrix[minI][k] * clusterSizes[minI] + currentMatrix[minJ][k] * clusterSizes[minJ]) /
                    newClusterSize;
...
export function neighborJoining(distanceMatrix) {
    const { labels, matrix, speciesIds } = distanceMatrix;
    const n = labels.length;
    if (n <= 1) {
        return {
...
export function buildPhylogeny(algorithm, species, characteristics, options = {}) {
    const warnings = [];
...
//# sourceMappingURL=phylogeny.js.map