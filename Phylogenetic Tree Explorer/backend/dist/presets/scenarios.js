import { run, saveDatabase } from '../db/database.js';
...
function createStandardVertebrate() {
    return {
        species: [
            {
                name: '七鳃鳗',
...
function createConvergentEvolution() {
    return {
        species: [
            {
                name: '金枪鱼',
...
function createMissingFossilData() {
    return {
        species: [
            {
                name: '现代马',
...
function createPolyphyleticGroup() {
    return {
        species: [
            {
                name: '蚯蚓',
...
export function loadScenarioToDatabase(scenarioId) {
    ...
    for (const sp of scenarioData.species) {
        const result = run(`INSERT INTO species (name, latin_name, kingdom, phylum, class, "order", family, genus, species)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
            sp.name,
            sp.latinName,
            sp.taxonomy.kingdom || '',
        ...
        for (const f of sp.features) {
            run('INSERT INTO features (species_id, feature_name, feature_value, category) VALUES (?, ?, ?, ?)', [speciesId, f.name, String(f.value), f.category]);
            run('INSERT INTO characteristic_matrix (species_id, feature_name, feature_value) VALUES (?, ?, ?)', [speciesId, f.name, f.value]);
