...
async function seed() {
  ...
  const count = db.get('SELECT COUNT(*) as c FROM pets');
  if (count && count.c > 0) {
    console.log('Database has data; skipping seed. Delete data/pets.db to reset.');
    return;
  }
  ...
  db.transaction(() => {
    for (const p of petList) {
      ...
      const records = defaultRecordsFor(petId, p.species, offsetDays);
      for (const r of records) {
        db.run(recSql, [
          r.pet_id, r.type, r.title, r.description, r.hospital,
          r.doctor, r.record_date, r.next_visit
        ]);
      }
    }
  });

  console.log('Seed data inserted:', petList.length, 'pets');
}

function defaultRecordsFor(petId, species, offsetDays) {
  const base = [
    { pet_id: petId, type: 'vaccine', title: '疫苗接种',
      description: '完成常规疫苗注射，状态良好。',
      hospital: '宠物医院总院', doctor: '李医生',
      record_date: offsetDays(-20), next_visit: offsetDays(160) },
    { pet_id: petId, type: 'checkup', title: '例行体检',
      description: '血液、生化指标正常，精神状态佳。',
      hospital: '宠物医院分院', doctor: '王医生',
      record_date: offsetDays(-10), next_visit: offsetDays(80) },
    { pet_id: petId, type: 'deworm', title: '体内驱虫',
      description: '按时口服驱虫药，无不良反应。',
      hospital: '宠物医院分院', doctor: '王医生',
      record_date: offsetDays(-5), next_visit: offsetDays(85) }
  ];
  if (species === '猫咪') {
    base.push({
      pet_id: petId, type: 'neuter', title: '绝育手术',
      description: '手术顺利，术后恢复良好，饮食正常。',
      hospital: '宠物医院总院', doctor: '李医生',
      record_date: offsetDays(-60), next_visit: null
    });
  }
  return base;
}
